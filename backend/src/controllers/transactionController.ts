...
class TransactionController {
  ...
  public async deposit(
    ...
    try {
      ...
      return res
        .status(200)
        .send({ status: "success", message: "Deposit successful", wallet });
    } catch (error) {
  ...
  public async convert(
    ...
    try {
      ...
      if (DtoSc) {
        // USD to smartCoin conversion
        if (wallet.usd < amount) {
          return res
            .status(400)
            .send({ status: "error", message: "Insufficient balance" });
        }
  ...
  public async withdraw(
    ...
    try {
      ...
      return res
        .status(200)
        .send({ status: "success", message: "Withdraw successful", wallet });
    } catch (error) {
  ...
  public async list_by_user(
    ...
    try {
      ...
      return res.status(200).send({
        status: "success",
        transactions: selectedTransactions.sort(
